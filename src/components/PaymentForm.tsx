    ...
    <div className="min-h-screen p-4 md:p-8 relative" style={{ 
      ...
      <div className="fixed top-0 left-0 w-full h-1 bg-gray-200 z-50">
        <div 
          ...
          style={{ 
            width: currentStep === 'shipping' ? '33.33%' : 
                   currentStep === 'addons' ? '66.66%' : '100%'
          }}
        />
      </div>
                    ...
                    {formData.payment?.method !== 'check' && !formData.addOns.monthlyPlan && !formData.addOns.digitalExposure && (
                      <div className="mt-4 p-3 bg-muted/50 rounded-lg border border-muted">
                        <p className="text-xs text-muted-foreground">
                          {formData.currency === 'CAD' ? '2.4%' : '2.9%'} processing fee (${(calculateTotal() * (formData.currency === 'CAD' ? 0.024 : 0.029)).toFixed(2)}) applied to credit card payments. Processing fee is a standard charge from our payment provider.
                        </p>
                      </div>
                    )}
